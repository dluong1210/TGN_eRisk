...
class UserNeighborFinder:
    """
    NeighborFinder for a single USER's data.
    
    Builds from ALL conversations of a user.
    """
    
    def __init__(self, user_data, n_users: int, uniform: bool = False):
        """
        Args:
            user_data: UserData object containing all conversations
            n_users: Total number of users in dataset
        ...
        # Get all interactions from all conversations
        sources, dests, timestamps, post_ids = user_data.get_all_interactions()
        ...
        if len(sources) > 0:
            ...
            # Empty neighbor finder
            self.neighbor_finder = NeighborFinder(
                [[] for _ in range(n_users)], uniform=uniform
            )
    ...
    def get_temporal_neighbor(self,
                              ...
                              timestamps: np.ndarray,
                              n_neighbors: int = 20) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        return self.neighbor_finder.get_temporal_neighbor(
            source_nodes, timestamps, n_neighbors
        )
    
    def find_before(self, src_idx: int, cut_time: float) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        return self.neighbor_finder.find_before(src_idx, cut_time)


# Alias for backward compatibility
ConversationNeighborFinder = UserNeighborFinder
